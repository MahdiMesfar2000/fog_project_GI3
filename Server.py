...
def run_app():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        server_socket.bind(('192.168.1.9', 100))
        server_socket.listen()

        print(f"Server listening on {'192.168.1.9'}:{100}")
...
def process_images():
    num_nodes = 2
    node_addresses = [("192.168.1.9", 200), ("192.168.1.9", 300)]
            ...
            for future, image_path in futures:
                if future.done():
                    ...
                    if image_path in imageConnection:
                        formatted_response = [f"{item['object']}: {item['probability']}" for item in response]
                        imageConnection[image_path].send(bytes('\n'.join(formatted_response), 'utf-8'))
